document.addEventListener('DOMContentLoaded', function () {
    ...
    var carType = document.getElementById('vechicleType');

    // Set the src attribute
    imgElement.src = "../img/users/" + obj.photo

    fullNameInput.value = obj.name;
    phoneNumberInput.value = obj.contact;
    license.value = obj.license
    carType.value = obj.carType
...
const updateSettings = async (name, contact, photo) => {
    try {
        ...
        if (typeof (photo) == "object") {
            url = `http://localhost:4000/api/drivers/updateMePic`
            headers = {
                'Content-Type': 'multipart/form-data',
            }
        } else {
            url = `http://localhost:4000/api/drivers/updateMe`
            headers = {
...
export const updatePasswords = async (data) => {
    try {
        ...
        const res = await axios({
            method: 'PATCH',
            url: `http://localhost:4000/api/drivers/updatePassword`,
            headers: {
